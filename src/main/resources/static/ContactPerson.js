...
if(saveMode==1){
    ...
    if ([name].every(x => x !== '')) {
        ...
        //localStorage.setItem("table", document.getElementById("myDynamicTable"));
       /* console.log("documents "+ documents);
        if(sessionStorage.getItem("contactPerson")!=null)
        {var storageSavedItem=sessionStorage.getItem("contactPerson")+","+documents.toString();
        console.log("inside if");}
        else
        {var storageSavedItem=documents.toString();
        console.log("inside else");}

        var storageSavedItem=documents.toString();
        sessionStorage.setItem("contactPerson",documents.toString() );
*/
        /*CustomerPersons=documents;
    ...
    //sessionStorage.setItem("contactPerson",storageSavedItem);

    //console.log("storageSavedItem "+storageSavedItem);
    //sessionStorage.setItem("contactPerson",storageSavedItem );
    //sessionStorage.removeItem("contactPerson");
    //console.log("contactPerson ", sessionStorage.getItem("contactPerson"));
    saveMode=0;
}
...
function deleteRows() {
/*console.log("storedArray before delete ", storedArrayy);
    var storedElement=sessionStorage.getItem("contactPerson");
    for(var i=0;i<storedElement.split(",").length;i+=6)
    {
        var newarr=new Array();
        for(var j=i;j<i+6;j++){
            newarr.push(storedElement.split(",")[j]);
        }
        storedArrayy.push(newarr);
    }*/
    ...
    xhttp.open("GET", "/DeleteContactPerson?id="+dbidd);
...
function UpdateRow() {
    ...
    xhr.open("post", "/UpdateContactPerson");      // open connection
    xhr.send(data);
...
function validate() {
    var tablee = document.getElementById("personstable");
...
function onloading() {
   // var CustomerPersons=new Array();
    ...
    var managers=document.getElementById("result").value;
    managers=managers.toString().substr(0,managers.length-2).split(",");
    console.log("managers ",managers);
    if(managers!="")
    {
        for(var i=0;i<managers.length;i+=5)
        {
            CustomerPersons.push([managers[i],managers[i+1],managers[i+2],managers[i+3],managers[i+4]]);
